...
from HumanResources.models import PayrollReceiptProcessed
...
class PaySheetSearchEngine():
    ...
    def search_paysheet(self):
        ...
        if self.payroll_period is not None:
            query &= Q(payroll_period_id=self.payroll_period)

        results = PayrollReceiptProcessed.objects.filter(query)
